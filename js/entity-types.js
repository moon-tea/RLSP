	...
	root.RL.Entity.Types = {
		zombie: {
			...
			initialize: function() {
				...
				//RL.Actions.Resolvable.add(this, 'toldWherePlayerIs');
			...
			updatePlayerLastSeen: function() {
				...
				/*
				if(this.informedOfPlayerLocation()) {
					this.playerLastSeen = {
						x: this.game.player.x,
						y: this.game.player.y
					};
		...
		informer: {
			...
			playerLastSeen: false,
			//FIXME: hard coded, we will want to change this later
			closestAlly: {
				x: 56,
				y: 23
			},
			...
			initialize: function() {
				...
				//RL.Actions.Performable.add(this, 'inform_other');
				//RL.Actions.Performable.add(this, 'find_closest_ally');
			},
			...
			_update: function() {
                ...
                //FIXME: add run_away
                if(this.isAdjacent(this.game.player.x, this.game.player.y)) {
					return this.performAction('run_away', this.game.player);
				}
                */

				var destination;
				if(this.playerLastSeen) {
					destination = this.getNextPathTile(this.closestAlly.x, this.closestAlly.y);

					if(!destination) {
						// get next path tile ignoring furniture and entities
						destination = this.getNextPathTile(this.closestAlly.x, this.closestAlly.y, true);
						if(destination) {
							var _this = this;
							
							var door = this.game.furnitureManager.getFirst(destination.x, destination.y, function(door) {
								return !door.passable && _this.canPerformActionOnTarget('open', door);
							});
							...
							if(door && this.performAction('open', door)) {
								return true;
							}
