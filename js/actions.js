(function(root) {
	...
	var performableActions = {
        ...
        talk: {
            ...
            performAction: function(target, settings){
                console.log(target);
                console.log(target.chat[target.chat['jump']].text);
                console.log(target.chat['welcome']);
				this.game.console.logNextMessage(this, target);
				 
                //alert(target.chat[target.chat['jump']].answers[0].text);
                var buttonTexts = [];
                console.log(target.chat[target.chat['jump']].answers);
                target.chat[target.chat['jump']].answers.forEach( function(answer) {
                    console.log(answer);
                    buttonTexts.push({
                        text: answer.text,
                        click: function() {
                            target.chat.jump = answer.jump;
                            $( this ).dialog( "close" );                                
                        }
                    });
                });
                ...
                $(function() {
                    //$( "#answer0" ).text(target.chat[target.chat['jump']].answers[0].text);
                ...
                return {
					//message: target.chat[target.jump]
				};
			},
			getTargetsForAction: function(settings){
				var _this = this;
				var validTargetsSettings = {
					limitToFov: true,
					filter: function(target){
						return _this.canPerformActionOnTarget('talk', target, settings);
					}
				};
				var validTargetsFinder = new RL.ValidTargetsFinder(this.game, this, validTargetsSettings);
				return validTargetsFinder.getValidTargets();
