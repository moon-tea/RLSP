(function(root) {
	...
	var performableActions = {
        ...
        lock: {
            ...
            canPerformActionOnTarget: true,
            performAction: function(target){
                this.game.console.logLock(this, target);
                return true;
            },
            getTargetsForAction: makeAdjacentTargetsFinder('lock')
        },
        unlock: {
            canPerformAction: true,
            canPerformActionOnTarget: true,
            performAction: function(target){
                this.game.console.logUnlock(this, target);
                return true;
            },
            getTargetsForAction: makeAdjacentTargetsFinder('unlock')
        },
		push: {
			canPerformAction: true,
			canPerformActionOnTarget: true,
	...
	var resolvableActions = {
		...
		open: {
			...
			canResolveAction: function(source, settings){
				//if it isn't locked
                if(!this.locked) {
                    //if it isn't open you can open it
                    return !this.open;
                }
                this.game.console.logCanNotOpenItem(source, this);
                return false;
			},
		...
		close: {
			...
			canResolveAction: function(source, settings){
				//if this is already closed, we can't close it
                if(!this.open){
					return false;
				}
			...
			resolveAction: function(source, settings){
				this.passable = false;
				this.blocksLos = this.whenClosedBlockLos;
				this.open = false;
				this.char = this.closedChar;
				return true;
			},
		},
		lock: {
			...
			canResolveAction: function(source, settings){
				//returns true if locked = false, this means you can lock it
                if(!this.open) {
                    return !this.locked;
                }
            },
		...
		unlock: {
			...
			canResolveAction: function(source, settings){
				//if this is not locked, we can't unlock it
                if(!this.locked){
					return false;
				}				
				return true;
